...
def _make_ssh_mock(port: int = 8888, port_in_use: bool = False) -> MagicMock:
    ...
    stdout_port     = MagicMock(); stdout_port.readline.return_value = f"{port}\n"
    stdout_netstat  = MagicMock()
    stdout_netstat.read.return_value = (
        f"tcp 0 0 0.0.0.0:{port}".encode() if port_in_use else b""
    )
    stdout_fuser    = MagicMock(); stdout_fuser.channel.recv_exit_status.return_value = 0
    stdout_oom      = MagicMock(); stdout_oom.channel.recv_exit_status.return_value = 0
...
class TestSpawnerStart:
    ...
    async def test_start_success_returns_ip_port(self, mock_spawner):
        ...
        with (
            ...
            patch.dict(spawner_module.hub_settings, {"hub_api_url": "http://hub:8081/hub/api"}),
        ):
...
class TestSpawnerStop:
    ...
    async def test_stop_sends_sigterm_and_sigkill(self, mock_spawner, store):
        ...
        store.set_session(
            "test_user",
            vm_ip="192.168.1.100", pid="42", port=8888,
            status="running", group_name="team_alpha",
        )
    ...
    async def test_stop_removes_session_from_store(self, mock_spawner, store):
        store.set_session(
            "test_user",
            vm_ip="192.168.1.100", pid="42", status="running",
            group_name="team_alpha",
        )
        mock_ssh = MagicMock()
        mock_ssh.exec_command.return_value = (None, MagicMock(
    ...
    async def test_stop_post_hook_called(self, mock_spawner, store):
        store.set_session("test_user", vm_ip="192.168.1.100", group_name="team_alpha",
                          status="running")
        hook_called = []
...
class TestSpawnerPoll:
    ...
    async def test_poll_running_returns_none(self, mock_spawner, store):
        store.set_session("test_user", vm_ip="192.168.1.100", pid="42",
                          group_name="team_alpha", status="running")
        mock_spawner._pid_file = "/tmp/jupyterhub-test_user.pid"
...
class TestJsonFallback:
    ...
    def test_get_server_info_falls_back_to_json(self, mock_spawner, store):
        ...
        with (
            ...
            patch.object(mock_spawner, "_load_mapping_fallback",
                         return_value=fallback_mapping),
        ):
            info = mock_spawner._get_server_info("team_alpha")

        assert info["server_ip"]  == "10.0.1.99"

    def test_get_server_info_raises_if_group_missing_everywhere(self, mock_spawner, store):
